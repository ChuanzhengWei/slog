...
#                 print(f"{region_type}, {start}, {end}")
                if region_type == 'L': 
                    segment = seq[start-1:end]
                    final_seq.append(segment.lower())
                else: 
                    segment = seq[start-1:end]
...
def main():
    parser = argparse.ArgumentParser(description="Mask FASTA genome based on repeat positions.")
